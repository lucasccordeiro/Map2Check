#!/usr/bin/env python2
#./bin/crabllvm.py -m 64 test.c --crab-track=arr --crab-add-invariants=all -o test.crab.bc
...
# Check if LD_LIBRARY_PATH has path to libCrab.so
libpath = root+"/lib:"
if not libpath in os.environ['LD_LIBRARY_PATH']:
	print os.environ['LD_LIBRARY_PATH']
	os.system("export LD_LIBRARY_PATH="+str(libpath)+"$LD_LIBRARY_PATH")

# export LD_LIBRARY_PATH=/home/map2check/devel_tool/mygitclone/release/bin/inv_gen_tools/lib:$LD_LIBRARY_PATH
...
def getClang (is_plus_plus):
    ...
    if is_plus_plus:
        ...
        #cmd_name = which (['clang-mp-3.8', 'clang-3.8', 'clang'])
        cmd_name = os.path.join (root, "../clang")
    if cmd_name is None:
...
def getOptLlvm ():
    ...
    cmd_name = os.path.join (root, "../opt")
